...
//
//
//  Copyright (c) 2014 Bluetooth SIG. All rights reserved.
//
...
/*!
 *  @method writeValueNoResponse:
 *
 *  @param characteristic Characteristic to write to
 *  @param data Data to write to peripheral
 *  @param p CBPeripheral to write to
 *
 *  @discussion Alternative routine for writeValue request, writes without requiring an ATT response and without feedback. It converts integer into
 *  CBUUID's used by CoreBluetooth. It then searches through the peripherals services to find a
 *  suitable service, it then checks that there is a suitable characteristic on this service.
 *  If this is found, value is written. If not nothing is done.
 *
...
-(void) writeValueNoResponse:(CBCharacteristic *)characteristic p:(CBPeripheral *)p data:(NSData *)data {

    NSLog(@"Sending write without response command to characteristic %s of service %s", [self CBUUIDToString:characteristic.UUID], [self CBUUIDToString:characteristic.service.UUID]);
 ...
 */
//VGAO
/*-(void) getServiceFromPeripheral:(CBPeripheral *)p
{
    NSLog(@"Searching for services");
    [p discoverServices:nil]; // Discover all services without filter
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error
{
    if (!error)
    {
        NSLog(@"test Services of peripheral with UUID : %s found\r\n",[self UUIDToString:peripheral.UUID]);
