...
static bool proximity_monitoring_connected = false;
static bool rssi_polling_required = false;

static bool alarm_on_lost_link = false;
...
static char* PROXIMITY_MONITORING_SERVICE_UUID = "3E099910-293F-11E4-93BD-AFD0FE6D1DFD";

// Characteristic UUIDs
static const char* ALERT_LEVEL_UUID = "0x2A06";
static const char* TX_POWER_LEVEL_UUID = "0x2A07";
static const char* CLIENT_PROXIMITY_CHARACTERISTIC_UUID = "0x3E099911-293F-11E4-93BD-AFD0FE6D1DFD";
...
static const int INDEX_PROXIMITY_MONITORING_SERVICE = 4;
...
const char *parse_characteristic_uuid(const char *uuid)
{
    const uuidnames_t characteristicUUID[] = { { 0x2a43, "ALERT_CATEGORY_ID" }, { 0x2A42,
            "ALERT_CATEGORY_ID_BIT_MASK" }, { 0x2A06, "ALERT_LEVEL" }, { 0x2A44,
            "ALERT_NOTIFICATION_CONTROL_POINT" }, { 0x2A3F, "ALERT_STATUS" }, { 0x2A01,
            "GAP_APPEARANCE" }, { 0x2A19, "BATTERY_LEVEL" }, { 0x2A49, "BLOOD_PRESSURE_FEATURE" }, {
            0x2A35, "BLOOD_PRESSURE_MEASUREMENT" }, { 0x2A38, "BODY_SENSOR_LOCATION" }, { 0x2A22,
            "BOOT_KEYBOARD_INPUT_REPORT" }, { 0x2A32, "BOOT_KEYBOARD_OUTPUT_REPORT" }, { 0x2A33,
            "BOOT_MOUSE_INPUT_REPORT" }, { 0x2A2B, "CURRENT_TIME" }, { 0x2A08, "DATE_TIME" }, {
            0x2A0A, "DAY_DATE_TIME" }, { 0x2A09, "DAY_OF_WEEK" }, { 0x2A00, "GAP_DEVICE_NAME" }, {
            0x2A0D, "DST_OFFSET" }, { 0x2A0C, "EXACT_TIME_256" }, { 0x2A26,
            "FIRMWARE_REVISION_STRING" }, { 0x2A51, "GLUCOSE_FEATURE" }, { 0x2A18,
            "GLUCOSE_MEASUREMENT" }, { 0x2A34, "GLUCOSE_MEASUREMENT_CONTEXT" }, { 0x2A27,
            "HARDWARE_REVISION_STRING" }, { 0x2A39, "HEART_RATE_CONTROL_POINT" }, { 0x2A37,
            "HEART_RATE_MEASUREMENT" }, { 0x2A4C, "HID_CONTROL_POINT" },
            { 0x2A4A, "HID_INFORMATION" }, { 0x2A2A,
                    "IEEE_11073_20601_REGULATORY_CERTIFICATION_DATA_LIST" }, { 0x2A36,
                    "INTERMEDIATE_BLOOD_PRESSURE" }, { 0x2A1E, "INTERMEDIATE_TEMPERATURE" }, {
                    0x2A0F, "LOCAL_TIME_INFORMATION" }, { 0x2A29, "MANUFACTURER_NAME_STRING" }, {
                    0x2A21, "MEASUREMENT_INTERVAL" }, { 0x2A24, "MODEL_NUMBER_STRING" }, { 0x2A46,
                    "NEW_ALERT" }, { 0x2A04, "GAP_PERIPHERAL_PREFERRED_CONNECTION_PARAMETERS" }, {
                    0x2A02, "GAP_PERIPHERAL_PRIVACY_FLAG" }, { 0x2A50, "PNP_ID" }, { 0x2A4E,
                    "PROTOCOL_MODE" }, { 0x2A03, "GAP_RECONNECTION_ADDRESS" }, { 0x2A52,
                    "RECORD_ACCESS_CONTROL_POINT" }, { 0x2A14, "REFERENCE_TIME_INFORMATION" }, {
                    0x2A4D, "REPORT" }, { 0x2A4B, "REPORT_MAP" },
            { 0x2A40, "RINGER_CONTROL_POINT" }, { 0x2A41, "RINGER_SETTING" }, { 0x2A4F,
                    "SCAN_INTERVAL_WINDOW" }, { 0x2A31, "SCAN_REFRESH" }, { 0x2A25,
                    "SERIAL_NUMBER_STRING" }, { 0x2A05, "GATT_SERVICE_CHANGED" }, { 0x2A28,
                    "SOFTWARE_REVISION_STRING" }, { 0x2A47, "SUPPORTED_NEW_ALERT_CATEGORY" }, {
                    0x2A48, "SUPPORTED_UNREAD_ALERT_CATEGORY" }, { 0x2A23, "SYSTEM_ID" }, { 0x2A1C,
                    "TEMPERATURE_MEASUREMENT" }, { 0x2A1D, "TEMPERATURE_TYPE" }, { 0x2A12,
                    "TIME_ACCURACY" }, { 0x2A13, "TIME_SOURCE" }, { 0x2A16,
                    "TIME_UPDATE_CONTROL_POINT" }, { 0x2A17, "TIME_UPDATE_STATE" }, { 0x2A11,
                    "TIME_WITH_DST" }, { 0x2A0E, "TIME_ZONE" }, { 0x2A07, "TX_POWER" }, { 0x2A45,
                    "UNREAD_ALERT_STATUS" }, { -1, NULL }, };
...
const char *parse_descriptor_uuid(const char *uuid)
{
    const uuidnames_t descriptorUUID[] =
            { { 0x2905, "Characteristic Aggregate Format" }, { 0x2900,
                    "Characteristic Extended Properties" }, { 0x2904,
                    "Characteristic Presentation Format" }, { 0x2901,
                    "Characteristic User Description" }, { 0x2902,
                    "Client Characteristic Configuration" },
                    { 0x2907, "External Report Reference" }, { 0x2908, "Report Reference" }, {
                            0x2903, "Server Characteristic Configuration" },
                    { 0x2906, "Valid Range" }, { -1, NULL }, };
...
const char *parse_service_uuid(const char *uuid)
{
    const uuidnames_t serviceUUID[] = { { 0x1800, "Generic Access" },
            { 0x1801, "Generic Attribute" }, { 0x1802, "ImmediateAlert" }, { 0x1803, "Link Loss" },
            { 0x1804, "Tx Power" }, { 0x1805, "Current Time Service" }, { 0x1806,
                    "Reference Time Update Service" }, { 0x1807, "Next DST Change Service" }, {
                    0x1808, "Glucose" }, { 0x1809, "Health Thermometer" }, { 0x180A,
                    "Device Information" }, { 0x180D, "Heart Rate" }, { 0x180E,
                    ":Phone Alert Status Service" }, { 0x180F, "Battery Service" }, { 0x1810,
                    "Blood Pressure" }, { 0x1811, "Alert Notification Service" }, { 0x1812,
                    "Human Interface Device" }, { 0x1813, "Scan Parameters" }, { 0x1814,
                    "Running Speed and Cadance" }, { 0x1816, "Cycling Speed and Cadance" }, { -1,
            NULL }, };
...
void setLinkLossAlertLevel()
{
    ...
    if (!link_loss_connected) {
        qDebug()
                << "XXXX not yet connected to the link loss service so ignoring alert request from user";
        return;
    }
    ...
    qDebug()
            << "XXXX setting link loss service initial alert level with bt_gatt_write_value_noresp. link_loss_service_instance="
            << link_loss_service_instance;

    if (bt_gatt_write_value(link_loss_service_instance, alert_level_value_handle, 0, level,
            sizeof(level)) == EOK) {
        qDebug() << "XXXX BluetoothHandler::setLinkLossAlertLevel() - value written successfully";
    } else {
        qDebug() << "XXXX BluetoothHandler::setLinkLossAlertLevel() - errno=(" << errno << ") :"
                << strerror(errno);
    }
}
...
void readTxPowerLevel()
{
    ...
    errno = 0;
    int result = bt_gatt_read_value(dc->getTxPowerServiceInstance(),
            dc->getTxPowerLevelValueHandle(), 0, tx_power_level, sizeof(uint8_t), 0);
    if (result < 0) {
    ...
    qDebug() << "XXXX read TX power level. result=" << result << " : tx_power_level="
            << *tx_power_level << " : power_level_dbm=" << power_level_dbm;
}

void gatt_service_connected(const char *bdaddr, const char *service, int instance, int err,
        uint16_t connInt, uint16_t latency, uint16_t superTimeout, void *userData)
{
    ...
    qDebug() << "YYYY PROXIMITY_MONITORING_SERVICE_UUID:" << PROXIMITY_MONITORING_SERVICE_UUID;
    ...
    if (strcmp(service, LINK_LOSS_SERVICE_UUID) == 0) {
        // we connected to the link loss service so...
    ...
    if (strcmp(service, TX_POWER_SERVICE_UUID) == 0) {
        // we connected to the tx power service so...
    ...
    if (strcmp(service, PROXIMITY_MONITORING_SERVICE_UUID) == 0) {
        qDebug() << "XXXX Proximity Monitoring Service found - establishing handles";
        // we connected to the CUSTOM PROXIMITY MONITORING service SO...
        proximity_monitoring_connected = true;
        establishHandles(INDEX_PROXIMITY_MONITORING_SERVICE, instance);
        return;
    }
...
void establishHandles(int service_type, int instance)
{
    if (service_type < 1 || service_type > 4) {
        qDebug()
                << "XXXX invalid service type indicator passed to establishAlertLevelHandle function:"
                << service_type;
        return;
    }
...
// find the handle of the characteristics of interest for this service
    ...
    if (num_characteristics > -1) {
        ...
        characteristicList = (bt_gatt_characteristic_t*) malloc(
                num_characteristics * sizeof(bt_gatt_characteristic_t));
        ...
        for (int i = 0; i < characteristicListSize; i++) {
            qDebug() << "YYYY characteristic: uuid,handle,value_handle:"
                    << characteristicList[i].uuid << "," << characteristicList[i].handle << ","
                    << characteristicList[i].value_handle;
            if (strcmp(characteristicList[i].uuid, characteristic_uuids[service_type - 1]) == 0) {
                qDebug() << "YYYY found characteristic of interest for this service:"
                        << characteristicList[i].uuid;
                switch (service_type) {
                    case INDEX_IMMEDIATE_ALERT_SERVICE:
                        qDebug()
                                << "XXXX CONNECTED TO IMMEDIATE ALERT SERVICE - saving handles for "
                                << characteristicList[i].uuid;
                    ...
                    case INDEX_LINK_LOSS_SERVICE:
                        qDebug() << "XXXX CONNECTED TO LINK LOSS SERVICE - saving handles for "
                                << characteristicList[i].uuid;
                    ...
                    case INDEX_TX_POWER_SERVICE:
                        qDebug() << "XXXX CONNECTED TO TX POWER SERVICE - saving handles for "
                                << characteristicList[i].uuid;
                        ;
                        ...
                        break;
                    case INDEX_PROXIMITY_MONITORING_SERVICE:
                        qDebug()
                                << "XXXX CONNECTED TO PROXIMITY MONITORING SERVICE - saving handles for "
                                << characteristicList[i].uuid;
                        ;
                        dc->setClientProximityHandle(characteristicList[i].handle);
                        dc->setClientProximityValueHandle(characteristicList[i].value_handle);
                        dc->setProximityMonitoringServiceInstance(instance);
                        break;
                }
            } else {
                qDebug() << "YYYY other characteristic: uuid,handle,value_handle="
                        << characteristicList[i].uuid << "," << characteristicList[i].handle << ","
                        << characteristicList[i].value_handle;
            }
        }
...
void gatt_service_disconnected(const char *bdaddr, const char *service, int instance, int reason,
        void *userData)
{
    ...
    qDebug()
            << QString("XXXX gatt_service_disconnected: bdaddr:%1 : %2 - %3").arg(bdaddr_str).arg(
                    service_str).arg(parse_service_uuid(service));
    ...
    if (instance == dc->getProximityMonitoringServiceInstance()) {
        qDebug() << "XXXX reacting to proximity monitoring service disconnection";
        proximity_monitoring_connected = false;
        dc->setProximityMonitoringServiceInstance(0);
    }

}

void gatt_service_updated(const char *bdaddr, int instance, uint16_t connInt, uint16_t latency,
...
bt_gatt_callbacks_t gatt_callbacks = { gatt_service_connected, gatt_service_disconnected,
        gatt_service_updated };

BluetoothHandler::BluetoothHandler(QObject * obj) :
        QObject(obj), _localDeviceInfo(new LocalDeviceInfo(this)), _deviceListing(
                new DeviceListing(this)), _remoteDeviceInfo(new RemoteDeviceInfo(this))
{
    _handler = this;
    ...
    if (!Utilities::getOSVersion().startsWith("10.0")) {
        ...
        PROXIMITY_MONITORING_SERVICE_UUID = "0x3E099910-293F-11E4-93BD-AFD0FE6D1DFD";
    }
...
void BluetoothHandler::setRemoteDevice(const QString &address)
{
    ...
    DataContainer::getInstance()->addProximityDevice(
...
void BluetoothHandler::setAlertLevel(const AlertLevel alert_level)
{
    _alertLevel = alert_level;
    ...
    int ll_alert_service_instance = dc->getLinkLossServiceInstance();
    uint16_t ll_alert_level_value_handle = dc->getLinkLossAlertLevelValueHandle();

    qDebug()
            << "XXXX setting link lost service alert level with bt_gatt_write_value_noresp. alert_service_instance="
            << ll_alert_service_instance;
    qDebug() << "XXXX link lost service alert level value handle=" << ll_alert_level_value_handle;
    qDebug() << "XXXX link lost service alert level value       =" << level[0];
...
void BluetoothHandler::sendAlertRequest()
{
    ...
    if (!immediate_alert_connected) {
        qDebug()
                << "XXXX not yet connected to the immediate alert service so ignoring alert request from user";
        return;
    }
    ...
    int ia_alert_service_instance = dc->getAlertServiceInstance();
    uint16_t ia_alert_level_value_handle = dc->getImmediateAlertLevelValueHandle();

    qDebug()
            << "XXXX setting immediate alert service alert level with bt_gatt_write_value_noresp. alert_service_instance="
            << ia_alert_service_instance;

    if (bt_gatt_write_value_noresp(ia_alert_service_instance, ia_alert_level_value_handle, 0, level,
            sizeof(level)) == EOK) {
...
void BluetoothHandler::sendAlertMid()
{
    qDebug() << "XXXX BluetoothHandler::sendAlertMid()";
    setAlertLevel(BluetoothHandler::Mid);
}
...
void BluetoothHandler::sendClientProximityData(uint8_t proximity_indicator, int rssi)
{

    if (!proximity_monitoring_connected) {
        qDebug()
                << "XXXX not yet connected to the proximity monitoring service so ignoring request to share data";
        return;
    }

    qDebug()
            << QString("XXXX BluetoothHandler::sendClientProximityData(%1,%2)").arg(
                    proximity_indicator).arg(rssi);
    uint8_t proximity_data[] = { 0, 0 };
    proximity_data[0] = proximity_indicator + 1;
    proximity_data[1] = rssi;

    errno = 0;

    DataContainer* dc = DataContainer::getInstance();

    int pm_service_instance = dc->getProximityMonitoringServiceInstance();
    uint16_t pm_client_proximity_value_handle = dc->getClientProximityValueHandle();

    qDebug()
            << "XXXX sharing client proximity data with bt_gatt_write_value_noresp. pm_service_instance="
            << pm_service_instance;

    if (bt_gatt_write_value_noresp(pm_service_instance, pm_client_proximity_value_handle, 0,
            proximity_data, sizeof(proximity_data)) == EOK) {
        qDebug() << "XXXX BluetoothHandler::sendClientProximityData() - value written successfully";
    } else {
...
// connect to the immediate alert service
    ...
    if (function_result < 0) {
        qDebug()
                << "YYYY GATT connect service request IMMEDIATE_ALERT_SERVICE_UUID failed: "
                        + QString::number(errno) + " (" + QString(strerror(errno)) + ")";
        Utilities::alert(
                "GATT connect service request IMMEDIATE_ALERT_SERVICE_UUID failed: "
...
// connect to the link loss service
    ...
    if (function_result < 0) {
        qDebug()
                << "YYYY GATT connect service request LINK_LOSS_SERVICE_UUID failed: "
                        + QString::number(errno) + " (" + QString(strerror(errno)) + ")";
        Utilities::alert(
                "GATT connect service request LINK_LOSS_SERVICE_UUID failed: "
                        + QString::number(errno));
    } else {
...
// connect to the TX power service
    ...
    if (function_result < 0) {
        Utilities::alert(
                "GATT connect service request TX_POWER_SERVICE_UUID failed: "
                        + QString::number(errno));
    } else {
        qDebug() << "YYYY requested connection to TX_POWER_SERVICE_UUID OK";
    }

    // connect to the custom Proximity Monitoring service
    ...
    if (function_result < 0) {
        Utilities::alert(
                "GATT connect service request PROXIMITY_MONITORING_SERVICE_UUID failed: "
                        + QString::number(errno));
    } else {
        qDebug() << "YYYY requested connection to PROXIMITY_MONITORING_SERVICE_UUID OK";
    }
...
void BluetoothHandler::pollRssi()
{
    ...
    while (rssi_polling_required) {
        ...
        if (rssi_polling_required) {
            int result = bt_rdev_get_current_rssi(dc->getCurrentDevice(), rssi);
            if (result < 0) {
                qDebug() << "XXXX bt_rdev_get_current_rssi - errno=(" << errno << ") :"
                        << strerror(errno);
                rssi_polling_required = false;
                if (errno != ENODEV) {
                    emitSignalSetMessage(
                            QString("'%1' error %2 getting RSSI value").arg(strerror(errno)).arg(
                            errno));
                    qDebug() << "XXXX freeing rssi memory following error";
                    free(rssi);
                    return;
                } else {
                    emitSignalSetMessage(QString("Lost Link"));
                    emitSignalLostLink();
                    qDebug() << "XXXX freeing rssi memory following link loss";
                    free(rssi);
                    return;
                }
            }
            ...
            qDebug() << "XXXX checking: proximity_indicator=" << proximity_indicator
                    << " tracking_proximity_band=" << tracking_proximity_band
                    << " last_proximity_band_notified=" << last_proximity_band_notified;

            if (proximity_indicator == tracking_proximity_band) {
                qDebug() << "XXXX tracked proximity band not changed, count="
            ...
            qDebug() << "XXXX checking: proximity_band_change_notified="
                    << proximity_band_change_notified << " this_proximity_band_count="
                    << this_proximity_band_count << " last_proximity_band_notified="
                    << last_proximity_band_notified;
            if ((proximity_band_change_notified == 0
                    && this_proximity_band_count > (10 - dc->getRssiFluctuationSensitivity()))
                    || (last_proximity_band_notified == -1)) {
                qDebug() << "XXXX signalling proximity band change: proximity_band_change_notified="
                        << proximity_band_change_notified << " this_proximity_band_count="
                        << this_proximity_band_count << " last_proximity_band_notified="
                        << last_proximity_band_notified;
    ...
    qDebug() << "XXXX freeing rssi memory prior to exiting";
    free(rssi);
...
void BluetoothHandler::disconnect() {
    ...
    bt_gatt_disconnect_service(device_addr.toAscii().constData(), IMMEDIATE_ALERT_SERVICE_UUID);
    ...
    bt_gatt_disconnect_service(device_addr.toAscii().constData(), PROXIMITY_MONITORING_SERVICE_UUID);
}
